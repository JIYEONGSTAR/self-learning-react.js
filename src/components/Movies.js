//참고코드
//https://ing-yeo.net/2020/10/react-beginner-3/
...
const MoviesPage = () => {
  ...
  const [movies3, setMovies] = useState({ // 영화 정보를 담는 state
    dataㄱ: getMovies(), // 영화 정보
    pageSize: 3, // 한 페이지에 보여줄 아이템(영화목록) 개수
    currentPage: 1 // 현재 활성화된 페이지 위치
  });

  const handlePageChange = (page2) => {
    setMovies({ ...movies3, currentPage: page2 });
  };

  const { dataㄱ: dataㄴ, pageSize, currentPage } = movies3;
  const pagedMovies = paginate(dataㄴ, currentPage, pageSize); // 페이지 별로 아이템이 속한 배열을 얻어옴

  const { length: count } = movies3.dataㄱ;
  if (count === 0) 
